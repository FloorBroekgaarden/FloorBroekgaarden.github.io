...
<div class="fig-block">
  ...
  <div class="fig-caption">The growing stellar graveyard: gravitational-wave detections from O1 through O4b, visualized by mass. Orange dots are neutron stars, whereas blue dots are black holes; the stellar graveyard has grown from the ~3 (2 confident and one that was less confident) events in the first observing run in 2015 to over 390 candidates today in O4b! Note how sparsely populated the neutron star regime (orange dots at the low-mass end) remains by O4b. <em>Credit: <a href="https://www.linkedin.com/in/gabriele-vajente?miniProfileUrn=urn%3Ali%3Afsd_profile%3AACoAAC9AkkUB6fuz_cVOx8XZtT93Es2BJlzZ2L4">Gabriele Vajente (LinkedIn post)</a>.</em></div>
</div>
...
<ul>
  <li><strong>No empty gap between 3–5 M<sub>☉</sub></strong>. With GWTC-5, the models rule out a completely empty "lower mass gap." This is now very solidly established: binary black holes do form black holes in what was historically thought to be a forbidden mass range between 3 and 5 solar masses. This was initially thought to be empty based on (few) X-ray observations that showed no Black holes in this mass range. With the increase of black hole masses from gravitational waves (but also through GAIA and spectroscopic discoveries of black holes, we now have a growing and confident population of black holes with masses in the range 3-5 solar masses</li>
...
<h3>The χ<sub>eff</sub>–q Correlation: an Earlier Puzzle That Is Fading</h3>

<p>One result I find especially interesting — and somewhat reassuring — concerns the reported correlation between χ<sub>eff</sub> and mass ratio q. Since GWTC-2.0, several analyses had reported a trend where more unequal-mass systems (lower q) tend to have higher effective spins. GWTC-5.0 revisits this and finds <strong>decreased evidence that the <em>mean</em> of χ<sub>eff</sub> evolves with q</strong>, but <strong>increased evidence that its <em>width</em> does</strong> — compared to GWTC-4.0.</p>

<p>Why does this matter? The earlier reported mean-shift correlation puzzled the stellar evolution community, because it is genuinely difficult to explain with standard formation physics. In most isolated binary evolution models, χ<sub>eff</sub> is set by processes like angular-momentum transport, tidal synchronization, and stellar winds — while the mass ratio is determined earlier, by mass transfer and stellar evolution. These are largely independent channels, so a strong coupling between them is not naturally expected. Dynamical formation in dense clusters likewise tends to produce nearly isotropic spin distributions with no intrinsic link to mass ratio. Explaining the trend required either fine-tuned evolutionary pathways, selection effects, or multi-channel complexity (hierarchical mergers, metallicity-driven correlations, etc.).</p>
...
<ul>
  ...
  <li><strong>No PISN gap.</strong> The primary-mass distribution extends smoothly past 100 M<sub>☉</sub> with no clean cutoff. This is a strong hint that multiple formation channels are overlapping in the observed spectrum, with hierarchical mergers filling in any intrinsic pair-instability gap.</li>
  <li><strong>The χ<sub>eff</sub>–q mean correlation is weakening.</strong> The earlier reported trend (more unequal-mass systems tend to have higher χ<sub>eff</sub>) was hard to explain with standard formation models and may have been partly a statistical fluctuation. GWTC-5.0 finds the evidence for a mean shift is decreasing, which simplifies the picture — though the width of the χ<sub>eff</sub> distribution may still evolve with q.</li>
